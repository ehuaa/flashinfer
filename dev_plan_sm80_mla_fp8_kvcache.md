...
#### 0.6.10 融合 fp8-kv kernel vs "fp8 存储 + 离线 `.to(bf16)` + bf16 kernel"（两段式）实测

一个自然的替代方案：KV cache 仍存 fp8（省显存），但不改 kernel——每个 decode step
先用一次 PyTorch 原生 `.to(torch.bfloat16)` cast（SM80 支持 fp8→bf16 elementwise
cast）把整个 cache dequant 成 bf16，再送进优化前的 q bf16 / kv bf16 MLA kernel。
实测证明该方案全面劣于本分支的融合 kernel。

**实验设置**（A100, fa2, page=64, q=bf16；脚本 `/tmp/bench_fp8_fused_vs_offline_cast.py`）：
两条路径 KV 都以 fp8 驻留 HBM，量化/写入 fp8 的时间两边均不计。KV 按 scale-free
方式存储（bf16 值直接 cast 成 e4m3），Plan B 的裸 `.to()` 不乘 scale，融合路径
相应传 scale=1.0。`two-step` 列为 cast + bf16 kernel 在同一 lambda 内端到端计时。
两路输出**逐位一致**（max diff = 0）——e4m3→bf16 cast 是精确的，两条路径在完全
相同的 bf16 值上做 attention。

```
batch seq_len heads  kv GB | fused ms |  cast ms bf16krnl two-step | speedup
   16    1024    16   0.01 |   0.0496 |   0.0626   0.0475   0.1027 |   2.07x
   64    1024    16   0.04 |   0.1555 |   0.2079   0.1419   0.3444 |   2.22x
   16    4096    16   0.04 |   0.1163 |   0.2079   0.1065   0.3093 |   2.66x
   64    4096    16   0.15 |   0.5497 |   0.7913   0.4854   1.2721 |   2.31x
   16   16384    16   0.15 |   0.4815 |   0.7902   0.4255   1.2122 |   2.52x
   64   16384    16   0.60 |   2.1271 |   3.1202   1.8647   4.9934 |   2.35x
   16   65536    16   0.60 |   2.1900 |   3.1262   1.9208   5.0423 |   2.30x
   64   65536    16   2.42 |   8.4368 |  12.4702   7.3765  19.8230 |   2.35x
   16    1024   128   0.01 |   0.0897 |   0.0625   0.0813   0.1390 |   1.55x
   64    1024   128   0.04 |   0.2907 |   0.2081   0.2566   0.4606 |   1.58x
   16    4096   128   0.04 |   0.2319 |   0.2080   0.2081   0.4107 |   1.77x
   64    4096   128   0.15 |   1.0794 |   0.7918   0.9455   1.7321 |   1.60x
   16   16384   128   0.15 |   0.8899 |   0.7915   0.7834   1.5696 |   1.76x
   64   16384   128   0.60 |   4.2284 |   3.1253   3.6981   6.8155 |   1.61x
   16   65536   128   0.60 |   3.4524 |   3.1218   3.0225   6.1395 |   1.78x
   64   65536   128   2.42 |  16.8306 |  12.4692  14.7100  27.1547 |   1.61x
```

**结论：融合 kernel 快 1.55–2.66x**（heads=128 约 1.6–1.8x，heads=16 约 2.1–2.7x），
且省显存 3 倍。

1. **单次 cast 就占融合 kernel 的 ~74%**。64×64K：cast 12.5ms vs 融合 kernel 全程
   16.8ms。cast 是纯 memory-bound（读 1B + 写 2B = 3B/elem），PyTorch 原生 cast
   kernel 实测仅 ~580 GB/s；融合 kernel 只读 1B/elem，dequant 藏在本来就
   compute-bound 的流水线里几乎免费（§0.6.9 消融：ALU 仅 5.9%）。
2. **把 cast 优化到带宽极限也翻不了盘**：3B/elem @ ~1.4TB/s 下限约 5.2ms，
   两段式下限 14.7+5.2≈19.9ms，仍比融合慢 ~1.2x——两段式多付的整趟 bf16
   写回+重读（4B/elem）是结构性的。
3. **heads=16 加速比更高（~2.3–2.7x）**：bf16 kernel 随 heads 变便宜，cast 开销
   不变——KV 流量相对 attention 计算越"贵"，两段式越亏。
...
注：若 fp8 存储带 per-tensor scale，Plan B 还需在 cast 外补一趟 `mul_(scale)`
（另测约 7B/elem、~940 GB/s），劣势进一步扩大到 1.7–3.4x。
